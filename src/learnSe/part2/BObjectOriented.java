...
//知识点
//记忆
//    1.抽象类（特点，成员，abstract）
//    2.接口（成员特点，抽象类与接口区别）
//    3.权限修饰符（记清作用，注意projected的说明）
//    4.package（import作用）
//    5.内部类
//            一般成员内部类：定义，如何使用
//            私有内部类：通过外部类的public方法访问
//            静态内部类：外部类名调用new内部类创建对象，new习惯提前
//            局部内部类：属于方法的内部类，只能在方法内调用；如果要使用所在方法的局部变量，该变量必须加上final，JDK8取消了这个东西
//            匿名局部内部类：用在方法中，或者作为参入传入，本质是子类对象，重写父类或者接口的方法
//了解
//
//1.抽象类
//    1.特点
...
//    1.特点
//        1.内部类中可以直接使用外部类的成员，包括私有（因为内部类可以获得外部类的引用Outer.this）
//        2.外部类要使用内部类的成员，必须创建对象：Inner inner = new Inner()；外部类之外要访问则需要Outer.Inner inner = new Outer().new Inner()
//        3.和其他的类一样，成员可以定义成private，提供public方法访问
//        4.内部类本身访问权限不能超过外部类（亲测），这也很好理解，因为访问内部类的前提是访问外部类
//    2.私有成员内部类   用private修饰
//        如果内部类private,那么将无法在外部类外面访问它，只能通过外部类的public方法访问
//    2.静态成员内部类   用static修饰
//        外部类名.内部类名 对象名 = 外部类名.内部类对象; 相当于外部类名调用内部静态成员，习惯把new提前
//        Outer.Inner oi = new Outer.Inner();
//        如果是访问静态内部类的静态成员，Outer.Inner.staticMethod();即可
//    3.局部内部类（成员方法内的内部类）
//        局部内部类只能在定义所在的方法中访问
//        jdk8以前，局部内部类访问其所在方法的局部变量时，该变量必须是显式的final修饰的自定义常量
//            原因：
//                1.生命周期不同：方法执行结束出栈时，局部变量销毁，但是局部内部类对局部变量的引用依然存在，当局部内部类需要使用该局部变量时，就会出现非法引用
//                2.数据不同步：
//                    局部内部类并不是直接使用局部变量，而是通过自己的构造备份到内部，表面上看是一个变量，实际上并不是
...
//                    private int priInnerAge;
//                    public void getInAge() {
//                        System.out.println("OuterName:" + name);//内部类可以直接访问外部类成员，本质上是因为可以获取Outer.this
//                        System.out.println("PriInnerAge:" + priInnerAge);
//                    }
//                }
//                //如果内部类private，可以通过public方法访问该内部类，在外部类之外无法直接通过Outer.Inner inner = new Outer().new Inner();访问内部类
//                public void getPriInnerAge() {
//                    PriInnerClass inner = new PriInnerClass();
//                    inner.getInAge();
//                }
//
//                //静态成员内部类，看作静态成员即可
...
//                    }
//
//                调用toShow()方法时，匿名内部类作为参数传递（即在调用toShow()方法时，传入一个匿名局部内部类对showName()进行重写即可）
//                    public void toShow(ForInner forInner) {
//                        forInner.showName();
//                    }
//                }
