...
//        2.构造方法（作用，public修饰，没有返回值，重载，创建对象的完整步骤）
//        3.static关键字（特点2个，3种代码块）
//        4.继承（单继承多层继承，子类继承规则，super关键字-子父类创建对象执行顺序）
//        5.final（类不能被继承，方法不能被重写，值只能初始化一次）
//        6.多态（前提，子父类关系，好处弊端）
//了解
//1.面向对象思想
//    1.区别于面向过程
...
//            3.在堆内存创建对象
//            4.给对象中属性默认初始化值（0，0.0，false，null等）这一步很重要，给对应的属性根据类型赋默认值，这一步是在所有操作之前的
...
//        然后要先进行父类属性的初始化（这是必然的，因为子类可能要用到父类的成员）
